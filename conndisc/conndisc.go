...
func main() {
	...
	if e != nil {
		ll.Printf("%stag%s main_on_connect error basic:\nerror:%v\nconn:%v\n",
			exampid, tag, e.Error(), conn)
		if conn != nil && conn.ConnectResponse != nil {
		...
		ll.Fatalf("%stag:%s connsess:%s main_on_connect FATAL error:%v",
