...
func main() {
	...
	d := sngecomm.Dest()
	i := stompngo.Uuid()
	r := sngecomm.Subscribe(conn, d, i, "client")
	...
	// Issue the final ACK
	fmt.Println("Starting final ACK")
	sngecomm.Ack(conn, lmd.Message.Headers, i)
	fmt.Println("Ending final ACK")

	// Here we do *not* UNSUBSCRIBE, or DISCONNECT.  We just close the
	// network connection and quit.
